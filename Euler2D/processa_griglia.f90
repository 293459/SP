...
do j=1,ele(interf(i)%e1)%nnghbrs
    ...
    if (((ele(interf(i)%e1)%edge(j,1).eq.interf(i)%nodo1).and.((ele(interf(i)%e1)%edge(j,2).eq.interf(i)%nodo2))).or.( (ele(interf(i)%e1)%edge(j,1).eq.interf(i)%nodo2).and.((ele(interf(i)%e1)%edge(j,2).eq.interf(i)%nodo1)))) then
    interf(i)%edge_e1=j
    end if
