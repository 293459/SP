...
lambda_max = max(abs(utildeA)+sqrt(gam*Ta),abs(utildeB)+sqrt(gam*Tb)) !se si usa il modulo della velocità diffondo di più quindi uso solo u

! Flussi nel sistema ruotato con metodo di Lax-Friedrichs locale
fluxes(1) = 0.5*(utildeA*(pa+uconsl(1))+utildeB*(pb+uconsr(1))) - lambda_max*0.5*(uconsr(1)-uconsl(1))
