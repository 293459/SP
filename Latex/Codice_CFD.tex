...
\paragraph{Variabili conservative vs primitive (scelta del solutore).}
Il solutore \textbf{evolve nel tempo le variabili conservative} \texttt{ucons}
($\rho E,\rho,\rho u,\rho v$) e ricava \emph{localmente} le primitive
($u,v,a,P,T,S$) solo quando servono (calcolo dei flussi, condizioni al
contorno). La scelta non è casuale: i metodi a volumi finiti integrano la
forma \emph{di divergenza} $\partial_t U+\nabla\!\cdot F=0$, l'unica che
garantisce automaticamente le corrette \textbf{relazioni di salto di
Rankine--Hugoniot} attraverso gli urti (velocità d'onda corretta). Usare le
primitive come incognite evolute (forma non conservativa) darebbe invece
velocità d'urto sbagliate. Le primitive restano comode per leggere lo stato e
per imporre le BC, ma il \emph{bilancio} viaggia sempre sulle conservative.
Il legame teorico (forma conservativa $\Leftrightarrow$ corretti urti) è
discusso in \texttt{teoria/caratteristiche.md} (sez.~5).
...
\paragraph{Cosa e come imponiamo (scelta delle BC).}
La scelta segue il \emph{conteggio delle caratteristiche entranti} (vedi
\texttt{teoria/caratteristiche.md} \S8: \# BC $=$ \# caratteristiche entranti):
\begin{itemize}
  \item \textbf{Inlet subsonico:} entrano 2 caratteristiche ($\lambda_2,\lambda_3$)
        $\Rightarrow$ imponiamo \textbf{2 grandezze termodinamiche totali},
        $T^\circ_{\rm in}$ e $P^\circ_{\rm in}$ (file \texttt{inlet.txt}); la
        velocità è ricavata dall'invariante $W_1$ che \emph{risale} dall'interno.
  \item \textbf{Outlet subsonico:} entra 1 caratteristica ($\lambda_1=u-a<0$)
        $\Rightarrow$ imponiamo \textbf{1 grandezza}, la \textbf{pressione statica}
        $P_{\rm exit}$ (file \texttt{outlet.txt}); le altre due si estrapolano
        dall'interno. È una BC \emph{riflettente} (semplice ma genera onde
        acustiche spurie).
  \item \textbf{Inlet/outlet supersonico:} rispettivamente 3 / 0 caratteristiche
        entranti $\Rightarrow$ si impone tutto / nulla.
  \item \textbf{Parete:} vincolo di impermeabilità $u_n=0$ (la pressione a parete
        si ricava dallo stato interno).
\end{itemize}
Quindi i dati richiesti dal codice ($T^\circ,P^\circ$ all'inlet, $P$ all'outlet)
non sono una scelta arbitraria: sono \emph{esattamente} le grandezze associate
alle caratteristiche entranti nei due regimi subsonici.
