...
% ============================================================
% SEZIONE COMMENTATA (errore noto #5):
% Il processo iterativo di raffinamento della mesh (Mesh v1 -> v2) viene omesso
% perche' le simulazioni vengono condotte direttamente sulla mesh a convergenza.
% Spiegare come si e' arrivati a tale mesh non e' necessario ai fini dei risultati.
% Per ripristinare la sezione, sostituire \iffalse con \iftrue.
% ============================================================
\iffalse
\section{Raffinamento Mesh}

\subsection{Mesh v1 -- Raffinamento Iniziale}
\label{sec:presa_mesh_v1}

Una prima versione della mesh è stata generata con i parametri di raffinamento riportati in \autoref{tab:presa_mesh_v1}.

\begin{table}[H]
  \centering
  \caption{Parametri di raffinamento mesh -- versione v1.}
  \label{tab:presa_mesh_v1}
  \begin{tabular}{ll}
    \toprule
    Parametro & Valore \\
    \midrule
    \texttt{Field[2].LcMin} & $lc \times 0.01 = 0.01$ \\
    \texttt{Field[2].LcMax} & $lc \times 0.10 = 0.10$ \\
    \texttt{Field[2].DistMin} & $lc / 4 = 0.25$ \\
    \texttt{Field[2].DistMax} & $lc / 2 = 0.50$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
% \begin{figure}[H]
%   \centering
%   \includegraphics[width=0.8\textwidth]{SP/2-Rampa/mesh_v1.png}
%   \caption{Mesh v1 con raffinamento iniziale. Si osserva un infittimento in prossimità
%            delle rampe e del canale, tuttavia le zone di transizione tra la regione fitta e diradata della mesh è troppo distanti dalle pareti fisiche.}
%   \label{fig:mesh_v1}
% \end{figure}

\paragraph{Osservazioni critiche sulla mesh v1.}
Sebbene il raffinamento sia presente nelle regioni di interesse (rampe e labbro della presa),
i parametri \texttt{DistMin} risulta eccessivamente ampio andando ad infittire anche zone che non lo necessiterebbero. 
Si decide quindi di abbassare \texttt{DistMin} da 0.25 a 0.15.
Tuttavia questo non sarà sufficiente poiché occorrerà preoccuparsi di desrcivere adeguatamente la zona di interazione tra gli urti obliqui, che si trova a valle del punto 9 (labbro) e non è coperta da nessuna delle linee attrattori. %Evidentemente, non è possibile inserire un attrattore su una linea che non esiste (non c'è nessuna linea che attraversa la zona di interazione), pertanto si decide di inserire un secondo campo di tipo \texttt{Box} centrato sulla zona di interazione, con \texttt{LcMin} più piccolo rispetto al primo campo Attractor.
\fi
% ============================================================
% Fine sezione commentata (errore noto #5)
% ============================================================
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{../Images/fluent_doppiarampa_mach}
  \caption{Campo di numero di Mach nella presa a doppia rampa ($M_{\rm in} = 3$), ottenuto
           con ANSYS Fluent (schema di Roe, secondo ordine upwind, solutore density-based).
           Sono chiaramente visibili i due urti obliqui attaccati rispettivamente al piede
           della prima rampa e al ginocchio tra prima e seconda rampa. La struttura a ventaglio
           delle isobare conferma la compressione progressiva del flusso supersonico,
           con il numero di Mach che scende da $M \approx 3$ a monte a valori intorno a
           $M \approx 1.5$ nella sezione del canale di cattura.}
  \label{fig:campo_mach_doppia_rampa}
\end{figure}
...
Si presentano i campi ottenuti con il \textbf{codice di Eulero sviluppato}, utilizzando lo
\textbf{schema di Roe} come richiesto dalla traccia. Le grandezze sono adimensionali, riferite
alle condizioni totali di ingresso ($P^\circ_{\rm in}=1$, $T^\circ_{\rm in}=1$). Tutti i campi
sono mostrati a convergenza raggiunta (residui $\sim10^{-6}$).
...
\end{equation}
I minimi letti nei campi ($T/T^\circ = 0.357$ e $P/P^\circ = 0.0272$ nella zona di monte) coincidono
\textbf{esattamente} con questi valori: le condizioni al contorno sono imposte correttamente e il
codice è consistente.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.92\textwidth]{../Images/Eulero_rampa_mach.png}
  \caption{Campo del numero di Mach nella presa a doppia rampa (codice Euler, schema di Roe).
           A monte $M=3$ uniforme; i due urti obliqui staccati dalle rampe decelerano
           progressivamente la corrente fino a $M\approx1.5$, mantenendola ovunque
           \textbf{supersonica}.}
...
\begin{itemize}
  ...
  \item attraverso la doppia compressione il numero di Mach scende da $3$ a circa $1.5$
    (minimo $M\approx1.53$), \textbf{restando supersonico} su tutto il dominio: è esattamente la
    condizione che la geometria semplificata vuole garantire (natura iperbolica delle equazioni,
    nessuna caratteristica entrante dall'outlet);
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.92\textwidth]{../Images/Eulero_rampa_pressione.png}
  \caption{Campo di pressione adimensionale $P/P^\circ$ (codice Euler, schema di Roe).
           Pressione bassa e uniforme a monte ($0.0272$), che cresce a gradini attraverso i due
           urti obliqui fino al picco ($\approx0.21$) nella zona di focalizzazione degli urti
...
due urti (ogni urto è una compressione), raggiungendo il \textbf{massimo} ($\approx0.21$, ossia
quasi otto volte la pressione di monte) nella regione dove i due urti obliqui \textbf{convergono e
si focalizzano} in prossimità del labbro, all'imbocco del canale di cattura. È qui che la presa
realizza il suo scopo: \emph{recuperare pressione} comprimendo la corrente supersonica. A valle, nel
canale, la pressione si ridistribuisce per effetto delle riflessioni d'onda sulle pareti.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.92\textwidth]{../Images/Eulero_rampa_temperatura.png}
  \caption{Campo di temperatura adimensionale $T/T^\circ$ (codice Euler, schema di Roe).
           È l'immagine "speculare" del Mach: minima a monte ($0.357$, flusso veloce) e crescente
           attraverso la compressione fino a $\approx0.68$ nel canale.}
...
fino a $T/T^\circ\approx0.68$ nel canale. La coerenza incrociata fra i tre campi ($M$, $P$, $T$
...
I campi sono calcolati sulla griglia impiegata per l'analisi di convergenza (la più rada,
$\sim1150$ nodi): gli urti appaiono perciò \textbf{spalmati} su alcune celle (\emph{smearing}
numerico). Lo schema di \textbf{Roe}, meno dissipativo, li rende comunque più netti di quanto
farebbe Lax--Friedrichs. Trattandosi di un modello \textbf{inviscido}, non sono presenti strato
limite né separazioni: la bolla di ricircolo e l'interazione urto--strato limite osservabili nel
caso viscoso (Cap.~\ref{cap:fluent}) non compaiono qui.
