...
% ============================================================
\section{Workflow di Simulazione}
\label{sec:fluent_workflow}
% ============================================================

Per coerenza, l'intera procedura di impostazione delle simulazioni Fluent (comune ai due
casi LS59 e presa a doppia rampa) è raccolta in questo capitolo dedicato, evitando di
disperdere i dettagli operativi nei capitoli delle singole esercitazioni. Il flusso di
lavoro seguito è riassunto schematicamente in Figura~\ref{fig:fluent_workflow}.

\begin{figure}[H]
  \centering
  \begin{tikzpicture}[>=Stealth, node distance=0.55cm,
      box/.style={rectangle, rounded corners, draw=blue!55!black, fill=blue!5,
                  text width=10.5cm, align=center, font=\footnotesize, inner sep=5pt}]
    \node[box] (geo)   {\textbf{Import geometria}: file \texttt{.step} esportato da Gmsh};
    \node[box, below=of geo]   (scale) {\textbf{Scalatura} $\times 1000$ (\textit{Body Transformation} $\to$ \textit{Scale})};
    \node[box, below=of scale] (mesh)  {\textbf{Mesh} non strutturata \textit{Quad Dominant} + \textit{inflation layers} a parete};
    \node[box, below=of mesh]  (setup) {\textbf{Setup solver}: \textit{density-based}, $p_{\rm ref}=0$, energia ON, gas ideale, modello Spalart--Allmaras};
    \node[box, below=of setup] (bc)    {\textbf{Condizioni al contorno} (inlet/outlet secondo il regime) e viscosità $\mu$ ricavata da $Re$};
    \node[box, below=of bc]    (solve) {\textbf{Soluzione}: schema di Roe, 2° ordine \textit{upwind}, formulazione implicita};
    \node[box, below=of solve] (post)  {\textbf{Post-processing}: contour di $M$ e $p$, $y^+$, profili di strato limite, confronti};
    \foreach \a/\b in {geo/scale, scale/mesh, mesh/setup, setup/bc, bc/solve, solve/post}
      \draw[->, very thick, blue!55!black] (\a) -- (\b);
  \end{tikzpicture}
  \caption{Diagramma di flusso del processo di simulazione in ANSYS Fluent, comune ai casi
           LS59 e presa a doppia rampa. La versione Mermaid editabile è disponibile nel file
           \texttt{Mermaid/07\_fluent\_workflow.mmd}.}
  \label{fig:fluent_workflow}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.88\textwidth]{../Images/fluent_LS59_mach}
    \caption{Isolivelli del numero di Mach -- caso RANS Fluent, schiera LS59.}
    \label{fig:fluent_ls59_mach}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.88\textwidth]{../Images/fluent_LS59_pressione}
    \caption{Isolivelli di pressione statica -- caso RANS Fluent, schiera LS59.}
    \label{fig:fluent_ls59_pressione}
\end{figure}

Dall'isolivello del numero di Mach (figura~\ref{fig:fluent_ls59_mach}) è
possibile apprezzare la presenza dello strato limite sulla superficie della
paletta, che porta $M$ ad essere nullo a parete avendo posizionato il
baricentro della prima cella all'interno del sottostrato viscoso.
...
I valori di pressione (figura~\ref{fig:fluent_ls59_pressione}) devono essere
normalizzati rispetto alla pressione totale in ingresso per essere
confrontabili con i dati sperimentali e con i risultati del codice CFD.

\subsection{Profili di strato limite}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.88\textwidth]{../Images/fluent_LS59_stratolimite}
    \caption{Profili di strato limite lungo la parete della paletta LS59 --
    caso RANS Fluent. Ciascun profilo è estratto in una sezione trasversale
    alla corda; la variabile in ascissa è la componente di velocità
    tangenziale normalizzata, in ordinata la coordinata normale alla parete.}
    \label{fig:fluent_ls59_bl}
\end{figure}

I profili di strato limite mostrano l'evoluzione dello spessore e della
forma del profilo di velocità lungo la lunghezza della corda della paletta.
In prossimità del bordo di attacco il profilo è tipicamente laminare e
sottile; procedendo verso il bordo di fuga lo strato limite si inspessisce
e, per effetto dei gradienti di pressione avversi, può presentare
caratteristiche tipiche del regime turbolento simulato dal modello
Spalart--Allmaras.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.88\textwidth]{../Images/confronto_Mis_LS59}
    \caption{Confronto del numero di Mach isentropico a parete $M_{is}$ lungo
    il profilo LS59: dati sperimentali, codice Eulero Fortran (schema di Roe)
    e simulazione RANS Fluent (Spalart--Allmaras).
    Sull'intradosso e nella zona centrale dell'estradosso i tre approcci
    concordano bene; le discrepanze si concentrano in prossimità del bordo
    di fuga, dove la presenza di gradienti avversi di pressione e l'effetto
    del bordo di uscita a cuneo producono effetti viscosi non catturabili
    dal modello Eulero inviscido. Il modello RANS si avvicina maggiormente
    ai dati sperimentali in questa regione grazie alla modellazione della
    turbolenza con Spalart--Allmaras.}
    \label{fig:fluent_confronto_Mis_LS59}
\end{figure}

Come si osserva nel grafico, tutti i modelli si comportano all'incirca allo
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.88\textwidth]{../Images/confronto_pressione_doppia_rampa}
    \caption{Confronto della distribuzione di pressione adimensionale $p_w/p^{\circ}$
    sulla parete inferiore della presa a doppia rampa: dati sperimentali, codice
    Eulero Fortran (schema di Roe), simulazione Eulero Fluent (inviscido) e
    simulazione RANS Fluent (Spalart--Allmaras).
    I modelli inviscidi concordano bene tra loro e con i dati sperimentali nella
    zona delle rampe ($x < 0.5$); le discrepanze a valle degli urti ($x > 0.5$)
    sono attribuibili all'interazione urto--strato limite (SBLI), un fenomeno
    puramente viscoso non modellato dalle equazioni di Eulero. Il modello RANS
    Spalart--Allmaras cattura parzialmente questa separazione, avvicinandosi ai
    dati sperimentali nella regione downstream della presa.}
    \label{fig:fluent_confronto_pressione_doppia_rampa}
\end{figure}
