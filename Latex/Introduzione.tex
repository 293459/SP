\chapter{Configurazione del Sistema e Logica di Sviluppo}
...
\section{Passaggi Informatici per la Configurazione}
...
\begin{itemize}
    ...
    \item \textbf{Automazione Build (\texttt{tasks.json}):} Creazione di un task per la compilazione modulare che unisce i file sorgente
          (\texttt{main.f90}, \texttt{calcolo\_area.f90}, \texttt{calcolo\_baricentro.f90})
          generando l'eseguibile nella directory \texttt{bin/Debug}.
    \item \textbf{Setup Debugging (\texttt{launch.json}):} Collegamento del motore di debug di VS Code all'eseguibile \texttt{gdb.exe}
          tramite il percorso \texttt{C:/Strawberry/c/bin/gdb.exe}.
...
\section{Vantaggi rispetto a Code::Blocks}
...
\section{Dettagli dell'Attuazione}
